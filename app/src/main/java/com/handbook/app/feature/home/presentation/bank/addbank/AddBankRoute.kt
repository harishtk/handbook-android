...
internal fun AddBankRoute(
    ...
    val editBankId by viewModel.bankId.collectAsStateWithLifecycle()

    AddBankScreen(
        uiState = uiState,
        uiAction = viewModel.accept,
        isInEditMode = editBankId != 0L,
    ...
    LaunchedEffect(key1 = uiState) {
        if (uiState == AddBankUiState.AddBankSuccess) {
            context.showToast("Bank added")
    ...
    ObserverAsEvents(viewModel.uiEvent) { event ->
        when (event) {
            is AddBankUiEvent.ShowToast -> {
                context.showToast(event.message.asString(context))
            }
            AddBankUiEvent.OnNavUp -> {
                onNextPageLatest()
            }
        }
    }
    ...
    Scaffold(
        ...
        if (confirmDelete) {
            CustomConfirmDialog(
                ...
                onConfirm = {
                    confirmDelete = false
                    uiAction(AddBankUiAction.DeleteBank)
                }
            )
        }
    }
...
@Composable
private fun ColumnScope.AddBankFormLayout(
    modifier: Modifier = Modifier,
    uiState: AddBankUiState.AddBankForm,
    uiAction: (AddBankUiAction) -> Unit,
    isInEditMode: Boolean = false,
) {
    ...
    val enableNext = true
    Footer(
        text = if (isInEditMode) "Update Bank" else "Add Bank",
        enabled = enableNext,
        onClick = {
            var isValid = true
